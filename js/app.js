(() => {
    ...
    (() => {
        ...
        function calcHeightMenuElements(init = true) {
            const headerHeight = document.querySelector(".header").clientHeight;
            var menuHeader = document.querySelector(".menu-header");
            if (init) menuHeader.style.setProperty("--maxHeight", `${headerHeight + 60}px`); else menuHeader.style.removeProperty("--maxHeight");
        }
        const filterCategoriesBlock = document.querySelector(".filter-articles");
        let filterCategoriesElement = document.querySelector(".filter-categories");
        let filterCategories = 0;
        function setSelectedCategories() {
            const filterActiveButtons = document.querySelectorAll(".filter-articles__button.active");
            filterCategories = filterActiveButtons.length > 0 ? filterActiveButtons.length : 0;
            filterCategoriesElement.innerHTML = filterCategories;
        }
        function onClickFilterCategory() {
            const filterArticlesButtons = document.querySelectorAll(".filter-articles__button");
            filterArticlesButtons.forEach((filterArticlesButton => {
                filterArticlesButton.addEventListener("click", (function(e) {
                    if (!filterArticlesButton.classList.contains("active")) onChangeSelectedCategories(true); else onChangeSelectedCategories(false);
                    filterArticlesButton.classList.toggle("active");
                }));
            }));
        }
        function onChangeSelectedCategories(plus = true) {
            if (plus) filterCategories++; else filterCategories--;
            filterCategoriesElement.innerHTML = filterCategories;
        }
        function onHashChange(tabs) {
            window.addEventListener("hashchange", (function() {
                if (tabs.options.dynamicUrl) {
                    tabs.changeTabNumber();
                    tabs.changeActiveIndex();
                } else {
                    window.location.href = location.hash;
                    const activeHash = tabs.getHash();
                    const hashChange = true;
                    tabs.changeActiveIndex(activeHash, hashChange);
                }
            }));
        }
        function onClickFilterButton() {
            ...
            filterButton.addEventListener("click", (function(e) {
                filterCategoriesBlock.classList.add("open");
                document.querySelector("body").classList.add("menu-filter-open");
                toggleLockBody(false, fixedElements, delay);
            }));
        }
        function onClickFilterClose() {
            const iconFilterClose = document.querySelector("[data-filter-close]");
            iconFilterClose.addEventListener("click", (function(e) {
                filterCategoriesBlock.classList.remove("open");
                document.querySelector("body").classList.remove("menu-filter-open");
                toggleLockBody(true, fixedElements, delay);
            }));
        }
        function onHoverProductCards() {
            const cloudImages = document.querySelectorAll(".image-cloud,.product-leokids__component");
            cloudImages.forEach((cloudImage => {
                cloudImage.addEventListener("mouseover", (function(e) {
                    if (!cloudImage.closest(".product-leokids.hover")) cloudImage.closest(".product-leokids").classList.add("hover");
                }));
                cloudImage.addEventListener("mouseout", (function(e) {
                    if (cloudImage.closest(".product-leokids").classList.contains("hover")) if (!e.relatedTarget?.parentElement.classList.contains("product-leokids__button")) cloudImage.closest(".product-leokids").classList.remove("hover");
                }));
            }));
        }
        function offset(el) {
            let rect = el.getBoundingClientRect(), scrollLeft = window.pageXOffset || document.documentElement.scrollLeft, scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            return {
                top: rect.top + scrollTop,
                left: rect.left + scrollLeft
            };
        }
        function calcHeightBlogCategories() {
            const categoriesItems = document.querySelectorAll(".article-blog__categories");
            for (let i = 0; i < categoriesItems.length; i++) categoriesItems[i].style.height = "auto";
            if (document.querySelector(".page-articles") && categoriesItems.length > 1) if (window.innerWidth > 550) {
                let columns = window.innerWidth > 767.98 ? 3 : 2;
                calcHeightColumns(categoriesItems, columns);
            }
            if (document.querySelector(".page-article") && categoriesItems.length > 1) if (window.innerWidth < 1200) {
                let columns = window.innerWidth > 900 ? 3 : categoriesItems.length;
                calcHeightColumns(categoriesItems, columns);
            }
            if (document.querySelector(".page-search-others") && categoriesItems.length > 1) if (window.innerWidth > 479.98) {
                let columns = window.innerWidth > 767.98 ? 3 : 2;
                calcHeightColumns(categoriesItems, columns);
            }
        }
        function calcHeightColumns(categoriesItems, columns) {
            let heightArray = [];
            for (let i = 0; i < categoriesItems.length; i++) {
                const categoriesItemHeight = categoriesItems[i].clientHeight;
                heightArray.push(categoriesItemHeight);
                if (i > 0 && i % columns === 0 || !categoriesItems[i + 1]) {
                    let numberRow = Math.ceil(i / columns);
                    for (let j = 0; j < columns; j++) {
                        const categoriesItem = categoriesItems[j + columns * (numberRow - 1)];
                        if (categoriesItem) categoriesItem.style.height = `${Math.max(...heightArray) / 16}rem`;
                    }
                    heightArray = [];
                }
            }
        ...
        function scrollHeader() {
            if (isShow) {
                ...
                if (header.hasAttribute("data-show-timer")) showTimer = header.dataset.showTimer ? header.dataset.showTimer : 500;
            }
            animateHeaderOnScroll();
        }
        function animateHeaderOnScroll() {
            if (scrollTop > scrollPoint) !header.classList.contains("scroll") ? header.classList.add("scroll") : null; else header.classList.contains("scroll") ? header.classList.remove("scroll") : null;
            if (isShow) if (scrollTop >= showPoint) {
                if (scrollDirection === "down") header.classList.contains("show") ? header.classList.remove("show") : null; else !header.classList.contains("show") ? header.classList.add("show") : null;
                if (showTimer) timer = setTimeout((() => {
                    header.classList.contains("show") ? header.classList.remove("show") : null;
                }), showTimer);
            } else !header.classList.contains("show") ? header.classList.add("show") : null;
        }
        ...
        let productBody, productSlider, productSliderHeight, productInfo, productBodyStyles, productInfoCoords, coordLeft, coordValue;
        if (document.querySelector(".page-product.leo-kids") && window.innerWidth > 767.98) {
            productBody = document.querySelector(".product-main__body");
            productSlider = productBody.querySelector(".product-main__slider");
            productSliderHeight = productSlider.querySelector(".slider-product__slide").clientHeight;
            productInfo = document.querySelector(".info-product");
            productBodyStyles = getComputedStyle(productBody);
            productInfoCoords = offset(productInfo);
            coordLeft = productInfoCoords.left - productBody.getBoundingClientRect().left - parseInt(productBodyStyles.getPropertyValue("padding-left"));
            coordValue = productInfoCoords.top + productInfo.offsetHeight - productSliderHeight - parseInt(productBodyStyles.getPropertyValue("padding-bottom"));
        }
        function calcPositionSticky() {
            if (window.scrollY > productInfoCoords.top - 80 && window.scrollY < coordValue) {
                window.innerWidth;
                productSlider.setAttribute("style", `\n            position: fixed;\n            width: ${productSlider.offsetWidth / 16}rem;\n        `);
                productInfo.style.paddingLeft = `${coordLeft / 16}rem`;
                productBody.classList.add("scroll");
            } else {
                if (window.scrollY < productInfoCoords.top - 80) {
                    productSlider.removeAttribute("style");
                    productInfo.style.paddingLeft = "0";
                    productBody.classList.remove("scroll");
                }
                if (window.scrollY > coordValue) productSlider.setAttribute("style", `\n                position: absolute;\n                top: auto;\n                bottom: ${parseInt(productBodyStyles.getPropertyValue("padding-bottom")) / 16}rem;\n                width: ${productSlider.offsetWidth / 16}rem;\n            `);
            }
        }
        ...
        function initMasonry(masonryGrid) {
            const gridItems = masonryGrid.querySelectorAll(".grid-item");
            gridItems.forEach((gridItem => {
                gridItem.style.height = gridItem.clientHeight + "px";
            }));
            let item = masonryGrid.querySelector(".grid-item");
            ...
            let arrayOfItems = Array.prototype.slice.call(document.querySelectorAll(".grid-item"));
            let trackHeights = {};
            let sum = 0;
            arrayOfItems.forEach((function(item) {
                let thisIndex = arrayOfItems.indexOf(item);
                let thisColumn = thisIndex % columnWidth;
                if (typeof trackHeights[thisColumn] == "undefined") trackHeights[thisColumn] = 0;
                ...
                if (thisIndex - columnWidth >= 0) {
                    let getItemAbove = document.querySelector(`.grid-item:nth-of-type(${thisIndex - columnWidth + 1})`);
            ...
            for (const key in trackHeights) sum += trackHeights[key];
            masonryGrid.style.maxHeight = `${sum / 3 + 40}px`;
        }
        function destroyMasonry(masonryGrid) {
            masonryGrid.removeAttribute("style");
            const gridItems = masonryGrid.querySelectorAll(".grid-item");
            gridItems.forEach((gridItem => {
        ...
        function onClickMenu(targetElement) {
            if (!document.body.classList.contains("menu-open")) {
                if (window.innerWidth < 1200) toggleLockBody(false, fixedElements, delay);
                openMenu(menu, iconMenu);
                if (targetElement.classList.contains("search__button") && window.innerWidth < 1200) setTimeout((() => {
                    document.body.classList.add("search-open");
                }), 100);
            } else {
                if (window.innerWidth < 1200) toggleLockBody(true, fixedElements, delay);
        ...
        function initLangMenu() {
            ...
            const langButtons = document.querySelectorAll(".list-lang__button");
            langButtons.forEach((button => {
                if (button.dataset.lang === langButton.getAttribute("data-lang")) button.closest(".list-lang__item").classList.add("hide");
            }));
        }
        function onClickLangMenu(targetElement) {
            const langBlock = document.querySelector(".lang-header");
            if (targetElement.classList.contains("list-lang__button") && !langBlock.classList.contains("slide")) {
                const langButton = document.querySelector(".lang-header__button");
                targetElement.getAttribute("data-lang");
                langButton.innerHTML = targetElement.innerHTML;
                langButton.setAttribute("data-lang", targetElement.getAttribute("data-lang"));
                langBlock.classList.remove("open");
                langBlock.classList.add("slide");
                setTimeout((() => {
                    langBlock.classList.remove("slide");
                    document.querySelector(".list-lang__item.hide").classList.remove("hide");
                    targetElement.closest(".list-lang__item").classList.add("hide");
                }), 350);
            }
            if (!langBlock.classList.contains("slide")) {
                langBlock.classList.add("slide");
        ...
        let _slideUp = (target, duration = 500, visibleHeight = 0, showmore = false) => {
            if (!target.classList.contains("_slide")) {
                ...
                if (showmore) target.style.height = visibleHeight ? `${visibleHeight}px` : `0px`;
                ...
                window.setTimeout((() => {
                    if (!showmore) target.style.display = "none"; else target.hidden = !visibleHeight ? true : false;
        ...
        class ShowMore {
            constructor(props, options) {
                this.showmoreClasses = {
                    classShowMoreOpen: "_showmore-open"
                };
                this.showmoreAttributes = {
                    attributeShowMore: "data-showmore",
                    attributeShowMoreSpeed: "data-showmore-speed",
                    attributeShowMoreMedia: "data-showmore-media",
                    attributeShowMoreСontent: "data-showmore-content",
                    attributeShowMoreButton: "data-showmore-button"
                };
                this.startOptions = {
                    type: "items",
                    visibleContent: 1,
                ...
                options ? this.showMoreOptions = {
                    ...this.startOptions,
                    ...options
                } : this.showMoreOptions = this.startOptions;
                if (props instanceof NodeList) props.forEach((showMoreItem => {
                    this.initShowMoreItem(showMoreItem);
                })); else this.initShowMoreItem(props);
            }
            initShowMoreItem(showMoreItem) {
                this.showMoreItem = showMoreItem;
                this.showMoreContent = Array.from(this.showMoreItem.querySelectorAll(`[${this.showmoreAttributes.attributeShowMoreСontent}]`)).filter((item => item.closest(`[${this.showmoreAttributes.attributeShowMore}]`) === showMoreItem))[0];
                this.showMoreButton = Array.from(this.showMoreItem.querySelectorAll(`[${this.showmoreAttributes.attributeShowMoreButton}]`)).filter((item => item.closest(`[${this.showmoreAttributes.attributeShowMore}]`) === showMoreItem))[0];
                ...
                this.heightsArray = this.getArrayHeights();
                this.generalHeight = this.heightsArray.generalHeight;
                this.initialHeight = this.heightsArray.initialHeight + this.showMoreOptions.gap / 2;
                ...
                if (!this.matchMedia || this.matchMedia.matches) if (this.initialHeight < this.generalHeight) {
            ...
            getMediaQuery() {
                ...
                let mediaQuery = "(" + breakpoint.type + "-width: " + breakpoint.value + breakpoint.unit + ")";
                return mediaQuery;
            }
            showMoreActions() {
                if (!this.showMoreContent.classList.contains("_slide")) if (this.showMoreItem.classList.contains("_showmore-open")) {
                ...
                } else if (this.showMoreOptions.addHeight === "all") {
                    this.showMoreButton.dataset.showmoreButton ? this.showMoreButton.innerHTML = this.showMoreButton.dataset.showmoreButton : this.showMoreButton.parentElement.style.display = "none";
                    ...
                    this.visibleHeight = this.showMoreContent.getBoundingClientRect().height;
                    this.visibleRows += this.showMoreOptions.addHeight;
                    this.addHeight = this.getHeight("add");
                    if (this.visibleHeight + this.addHeight + 10 >= this.generalHeight) {
                        this.showMoreButton.dataset.showmoreButton ? this.showMoreButton.innerHTML = this.showMoreButton.dataset.showmoreButton : this.showMoreButton.parentElement.style.display = "none";
                        _slideDown(this.showMoreContent, this.showMoreSpeed, this.visibleHeight, 0, true);
                        this.showMoreItem.classList.add("_showmore-open");
                    } else _slideDown(this.showMoreContent, this.showMoreSpeed, this.visibleHeight, this.addHeight, true);
                }
            }
            getArrayHeights() {
                let heightsArray = {};
                heightsArray.generalHeight = this.showMoreContent.getBoundingClientRect().height;
                this.showMoreType = this.showMoreOptions.type;
                if (this.showMoreType === "size") heightsArray.initialHeight = Number(this.showMoreOptions.visibleContent);
                if (this.showMoreType === "items") {
                    const initialHeight = this.getHeight("initial");
                    this.showMoreOptions.visibleContent === 1 ? heightsArray.initialHeight = initialHeight : heightsArray.initialHeight = initialHeight + this.showMoreOptions.gap * (this.showMoreOptions.visibleContent - 1);
                }
                return heightsArray;
            }
            getHeight(typeValue) {
                ...
                if (showMoreColumns) {
                    if (typeValue === "initial") if (this.showMoreOptions.visibleContent === 1) valueHeight = this.showMoreContent.children[0] ? this.showMoreContent.children[0].getBoundingClientRect().height : null; else for (let index = 0; index < showMoreColumns.length; index++) {
                        ...
                        if (Number.isInteger(columnIndex) && columnIndex <= this.showMoreOptions.visibleContent) {
                            ...
                            valueHeight += showMoreColumn.getBoundingClientRect().height;
                        }
                    }
                    if (typeValue === "add") if (this.showMoreOptions.addHeight === 1) valueHeight = this.showMoreContent.children[(this.visibleRows - 1) * this.showMoreOptions.columns] ? this.showMoreContent.children[(this.visibleRows - 1) * this.showMoreOptions.columns].getBoundingClientRect().height + this.showMoreOptions.gap : null; else for (let index = 0; index < showMoreColumns.length; index++) if (index >= (this.visibleRows - this.showMoreOptions.addHeight) * this.showMoreOptions.columns && index < this.visibleRows * this.showMoreOptions.columns) {
                        let columnIndex = (index + this.showMoreOptions.columns) / this.showMoreOptions.columns;
                        if (Number.isInteger(columnIndex) && columnIndex <= this.visibleRows) {
                            const showMoreColumn = showMoreColumns[index];
                            valueHeight += showMoreColumn.getBoundingClientRect().height + this.showMoreOptions.gap;
                        }
                    }
                }
                return valueHeight;
        ...
        function initShowmore() {
            if (document.querySelector(".articles__body")) if (window.innerWidth > 1880) new ShowMore(document.querySelector(".articles__body"), {
                visibleContent: 4,
                addHeight: 4,
                gap: 30,
                columns: 3
            }); else if (window.innerWidth > 1200 && window.innerWidth < 1880) new ShowMore(document.querySelector(".articles__body"), {
                visibleContent: 3,
                addHeight: 3,
                gap: 15,
                columns: 3
            }); else if (window.innerWidth > 767.98 && window.innerWidth < 1200) new ShowMore(document.querySelector(".articles__body"), {
                visibleContent: 4,
                addHeight: 4,
                gap: 16,
                columns: 3
            }); else if (window.innerWidth > 479.98 && window.innerWidth < 767.98) new ShowMore(document.querySelector(".articles__body"), {
                visibleContent: 6,
                addHeight: 6,
                gap: 16,
                columns: 2
            }); else new ShowMore(document.querySelector(".articles__body"), {
                visibleContent: 7,
                addHeight: 7,
                gap: 17,
                columns: 1
        ...
        function goTo() {
            ...
            function goToBlock(e) {
                ...
                if (targetBlockElement) if (typeof SmoothScroll !== "undefined") ; else {
                    document.body.classList.contains("menu-open") ? closeMenu(menu, iconMenu) : null;
                    let headerBlockHeight = document.querySelector(".header").offsetHeight;
                    window.scrollTo({
                        top: headerBlockHeight ? targetBlockElement.getBoundingClientRect().top + window.scrollY - headerBlockHeight : targetBlockElement.getBoundingClientRect().top + window.scrollY,
                        behavior: "smooth"
                    });
        ...
        function videoSlider() {
            if (document.querySelectorAll("video")) {
                ...
                for (let item = 0; item < videos.length; item++) {
                    const video = videos[item];
                    video.addEventListener("click", (function(e) {
                        const activeVideo = e.target;
                        controlVideo(null, activeVideo);
                    }));
                }
            }
        }
        function controlVideo(prevVideo, activeVideo, slideChange = false) {
            if (!slideChange) if (activeVideo.paused) playVideo(activeVideo); else stopVideo(activeVideo); else {
                if (prevVideo) stopVideo(prevVideo, true);
                if (activeVideo) playVideo(activeVideo, true);
            }
        }
        function playVideo(video, slideChange = false) {
            const poster = video.closest(".video").querySelector("[data-poster]");
            video.play();
            if (!slideChange) {
        ...
        function stopVideo(video, slideChange = false) {
            video.pause();
            if (!slideChange) video.closest(".video").classList.remove("play"); else setTimeout((() => {
                video.closest(".video").classList.remove("play");
            }), 1e3);
        }
        ...
        function initSliders() {
            if (document.querySelector(".slider-main")) {
                new Swiper(".slider-main", {
                    ...
                    modules: [ A11y, EffectFade, Keyboard, Navigation, Pagination ],
                    navigation: {
                        nextEl: ".main [data-arrow-next]",
                        prevEl: ".main [data-arrow-prev]"
                    },
                    pagination: {
                        clickable: true,
            ...
            if (document.querySelector(".slider-news")) {
                new Swiper(".slider-news", {
                    ...
                    navigation: {
                        nextEl: ".news [data-arrow-next]",
                        prevEl: ".news [data-arrow-prev]"
                    },
                    breakpoints: {
                        1760: {
                            slidesPerView: 4,
            ...
            if (document.querySelector(".slider-product")) new Swiper(".slider-product", {
                observer: true,
                observeParents: true,
                watchOverflow: true,
                slidesPerView: 1,
                ...
                pagination: {
                    clickable: true,
                    el: ".product-main [data-bullets]",
                    type: "bullets",
                    renderBullet: function(index, className) {
                        return `\n                        <button class=${className} aria-label="перейти к слайду ${index + 1}">\n                            <span></span>\n                        </button>\n                    `;
                    }
                },
                modules: [ A11y, EffectFade, Pagination, Thumb ],
                thumbs: {
                    swiper: {
                        el: ".thumbs-product",
                        slidesPerView: "auto",
                ...
                on: {
                    init: function(swiper) {
                        if (swiper.slides[0].querySelector("video")) {
                            const activeVideo = swiper.slides[0].querySelector("video");
                            setTimeout((() => {
                                activeVideo.setAttribute("muted", "false");
                                playVideo(activeVideo);
                            }), 500);
                        }
                        const thumbs = document.querySelectorAll(".thumbs-product__slide");
                        thumbs.forEach((thumb => {
                            thumb.addEventListener("click", (function(e) {
                                if (!swiper.el.classList.contains("slide-change")) {
                    ...
                    slideChange: function(swiper) {
                        swiper.el.classList.add("slide-change");
                        setTimeout((() => {
                            swiper.el.classList.remove("slide-change");
                        }), 1e3);
                        const prevVideo = swiper.slides[swiper.previousIndex].querySelector("video");
                        const activeVideo = swiper.slides[swiper.activeIndex].querySelector("video");
                        if (prevVideo || activeVideo) controlVideo(prevVideo, activeVideo, true);
                    }
                }
            });
            if (document.querySelector(".slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
                    slidesPerView: 4,
                    ...
                    modules: [ A11y, EffectFade, Keyboard, Navigation ],
                    navigation: {
                        nextEl: ".other-products [data-arrow-next]",
                        prevEl: ".other-products [data-arrow-prev]"
                    },
                    breakpoints: {
                        1441: {
                            slidesPerView: 4,
            ...
            if (document.querySelector(".leo-kids .slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
            ...
            if (document.querySelector(".champ .slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
            ...
            if (document.querySelector(".snack .slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
            ...
            if (document.querySelector(".health-foods .slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
            ...
            if (document.querySelector(".lactogon .slider-other-products")) {
                new Swiper(".slider-other-products", {
                    observer: true,
                    observeParents: true,
                    watchOverflow: true,
        ...
        function initSimpleBar() {
            simpleScroll = new SimpleBar(document.getElementById("thumbs"), {
                autoHide: true,
                clickOnTrack: true
            });
            const sliderProductHeight = document.querySelector(".slider-product").offsetHeight;
            let thumbsBlock = document.getElementById("thumbs");
            thumbsBlock.style.maxHeight = `${sliderProductHeight / 16}rem`;
        }
        class Spollers {
            constructor(props, options) {
            ...
            onActionSpoller(e) {
                if (e.target.hasAttribute("data-spoller") || e.target.closest("[data-spoller]")) {
                    if (document.querySelector(".lang-header.open")) document.querySelector(".lang-header").classList.remove("open");
        ...
        function initSpollers() {
            if (document.querySelector("#spollers-menu")) {
                const spollersMenu = document.querySelector("#spollers-menu");
                new Spollers(spollersMenu, {
                    mediaValue: "1200",
                    typeMedia: "max",
                    speed: 350
                });
        ...
        class Tabs {
            constructor(props, options) {
                this.startOptions = {
                    ...
                    unitValue: "em",
                    speed: 500,
                    dynamicUrl: tabNumber ? true : false,
                    animate: false
                };
                options ? this.options = {
                    ...this.startOptions,
            ...
            initTabs(tabsItem) {
                ...
                this.activeHash = !this.options.dynamicUrl ? this.getHash() : null;
                this.activeTabNumber = this.options.dynamicUrl ? tabNumber - 1 : null;
                this.activeIndex = null;
                this.activeTitle = this.tabsItem.querySelector("[aria-selected='true']");
                if (this.activeTitle) this.activeTitle.setAttribute("aria-selected", "false");
                this.changeActiveIndex();
                this.titles.forEach(((title, index) => {
                    if (this.panels[index]) {
                        this.title = title;
                        this.titleID = this.title.getAttribute("id");
                        this.panel = this.panels[index];
                        ...
                        if (index === this.activeIndex) this.title.setAttribute("aria-selected", "true"); else {
            ...
            changeActiveIndex(activeHash, hashChange) {
                if (this.options.dynamicUrl) this.tabsItemActive = this.activeIndex = this.activeTabNumber; else {
                    activeHash ? this.activeHash = activeHash : null;
                    hashChange ? this.hashChange = hashChange : null;
                    this.tabsItemActive = Number(this.activeHash[0]);
                    this.tabsIndex === this.tabsItemActive ? this.activeIndex = Number(this.activeHash[1]) : this.activeIndex = 0;
                }
                this.itemIndex = this.tabsItem.dataset.tabsIndex;
                if (this.hashChange) {
                    if (this.titles[this.activeIndex] && !this.titles[this.activeIndex].hasAttribute("hidden")) this.activeTitle = this.titles[this.activeIndex]; else this.activeTitle = this.titles[0];
            ...
            switchTabs() {
                if (this.panels.length > 0) this.panels.forEach(((panel, index) => {
                    if (this.titles[index]) if (this.titles[index].getAttribute("aria-selected") === "true") {
                        ...
                        if (!this.options.dynamicUrl) location.hash = `tab-${this.itemIndex}-${index}`; else tabNumber = index + 1;
            ...
            getHash() {
                if (location.hash.substring("tab")) this.hash = location.hash.replace("#", ""); else location.hash = this.hash = "tab-0-0";
                this.activeHashArray = [];
                if (this.hash.startsWith("tab-")) {
                    this.activeHashArray = this.hash.replace("tab-", "").split("-");
                    if (this.panels.length - 1 < Number(this.activeHashArray[1])) {
                        location.hash = this.hash = "tab-0-0";
                        this.activeHashArray = this.hash.replace("tab-", "").split("-");
                    }
        ...
        function initTabs() {
            if (document.querySelector(".products-onco [data-tabs]")) {
                const tabsProductsOnco = new Tabs(document.querySelector(".products-onco [data-tabs]"), {});
                onHashChange(tabsProductsOnco);
            }
            if (document.querySelector(".products-pharma [data-tabs]")) {
                const tabsProductsPharma = new Tabs(document.querySelector(".products-pharma [data-tabs]"), {});
                onHashChange(tabsProductsPharma);
            }
        }
        ...
        let app_productSlider, app_productSliderHeight, productInfoHeight;
        if (document.querySelector(".page-product.leo-kids") && window.innerWidth > 767.98) {
            app_productSlider = document.querySelector(".product-main__slider");
            app_productSliderHeight = app_productSlider?.querySelector(".slider-product__slide").clientHeight;
            productInfoHeight = document.querySelector(".info-product").offsetHeight;
        }
        ...
        if (document.querySelector(".menu-header") && window.innerWidth > 1200) calcHeightMenuElements();
        if (document.querySelector(".page-articles")) {
            if (document.querySelector(".filter-articles__button")) {
                if (document.querySelector(".filter-categories")) setSelectedCategories();
                onClickFilterCategory();
            }
            if (document.querySelector("[data-filter-button]")) onClickFilterButton();
            if (document.querySelector("[data-filter-close]")) onClickFilterClose();
        }
        if (document.querySelector(".page-category.leo-kids") || document.querySelector(".page-product.leo-kids")) onHoverProductCards();
        document.querySelector("header[data-scroll]") ? scrollHeader() : null;
        document.querySelector("[data-da]") ? initDynamicAdapt() : null;
        document.querySelector(".lang-header") && initLangMenu();
        document.querySelector("[data-goto]") ? goTo() : null;
        if (document.querySelector("#thumbs")) window.innerWidth > 1200 ? initSimpleBar() : null;
        document.querySelector("[data-spollers]") ? initSpollers() : null;
        if (document.querySelector(".page-product.leo-kids") && app_productSliderHeight < productInfoHeight && window.innerWidth > 767.98) calcPositionSticky();
        document.querySelector("[data-tabs]") ? initTabs() : null;
        document.querySelector("video") ? videoSlider() : null;
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (!targetElement.closest(".open") && document.querySelector(".open") && !document.querySelector(".open.slide")) if (window.innerWidth > 1200) if (document.body.classList.contains("menu-open")) closeMenu(menu, iconMenu); else document.querySelector(".open").classList.remove("open");
            if (!targetElement.closest(".lang-header") && document.querySelector(".lang-header.open") && !document.querySelector(".open.slide")) document.querySelector(".lang-header").classList.remove("open");
            if (targetElement.classList.contains("search__button")) if (window.innerWidth > 1200) document.querySelector(".search-main").classList.toggle("open"); else onClickMenu(targetElement);
            if (targetElement.closest(".lang-header__button") || targetElement.closest(".list-lang__button")) onClickLangMenu(targetElement);
            targetElement.getAttribute("aria-controls") === "menu" ? onClickMenu(targetElement) : null;
        }));
        window.addEventListener("load", (function(e) {
            if (!localStorage.getItem("leovit-cookies")) {
                ...
                cookiesButton.addEventListener("click", (function(e) {
                    ...
                    localStorage.setItem("leovit-cookies", "confirmed");
                }));
            }
            if (document.querySelector(".article-blog__categories")) calcHeightBlogCategories();
            if (typeof tabNumber !== "undefined" && document.querySelector("[data-tabs-titles]")) {
                const tabButtons = document.querySelectorAll("[data-tabs-titles] a");
                const tabContentElements = document.querySelectorAll("[data-tabs-body] > *");
                if (tabNumber <= 0 || tabNumber > tabButtons.length) tabNumber = 1;
                tabButtons.forEach(((_, index) => {
                    tabContentElements[index].style.display = "none";
                    if (tabNumber === 1) {
                        if (index === 0) showTabContent(0);
                    } else if (index + 1 === tabNumber) showTabContent(index);
                    function showTabContent(index) {
                        tabButtons[index].classList.add("active");
                        if (document.querySelector(".onco")) tabContentElements[index].style.display = "flex"; else tabContentElements[index].style.display = "block";
                    }
                }));
            }
            if (document.querySelector("[data-masonry]") && window.innerWidth > 1200) masonryGrids.forEach((masonryGrid => {
                initMasonry(masonryGrid);
            }));
            const mediaQuery = "(min-width: 75em)";
            const matchMedia = window.matchMedia(mediaQuery);
            matchMedia.addListener((function() {
                if (document.querySelector("[data-masonry]")) if (matchMedia.matches) masonryGrids.forEach((masonryGrid => {
                    initMasonry(masonryGrid);
                })); else masonryGrids.forEach((masonryGrid => {
                    destroyMasonry(masonryGrid);
                }));
                if (document.querySelector(".menu-header")) if (matchMedia.matches) calcHeightMenuElements(true); else calcHeightMenuElements(false);
            }));
            document.querySelector("[data-showmore]") ? initShowmore() : null;
        }));
        ...
        window.addEventListener("orientationchange", (function(e) {
            ...
            document.querySelector("[data-showmore]") ? initShowmore() : null;
            if (document.querySelector(".page-product.leo-kids") && window.innerWidth < 767.98) {
                document.querySelector(".product-main").classList.remove("scroll");
                document.querySelector(".product-main__slider").removeAttribute("style");
            }
            if (document.querySelector(".article-blog__categories")) calcHeightBlogCategories();
        }));
